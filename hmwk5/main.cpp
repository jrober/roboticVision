...
Point2f MatchingMethod(Mat searchArea, Mat templ, Point2f pt)
{
  ...
  while(!emptyFrame)
  {
      ...
      if (emptyFrame){
        ...
        for(int i = 0; i < previousPoints.size(); i++)
        {
          ...
          Point2f tmpCorrected = correctPoint(previousPoints[i], grayScale.cols, grayScale.rows, tmpW);
          Point2f srcCorrected = correctPoint(previousPoints[i], grayScale.cols, grayScale.rows, srcW);

          Rect templ_roi(tmpCorrected, tmpSize);
          Mat templ = grayScale(templ_roi);
        ...
        // reducedPrev = previousPoints;
        // reducedNext = nextPoints;
