...
Mat reduce(vector<Point2f> prevPtsNonReduced, vector<Point2f> nextPtsNonReduced, vector<Point2f>& prevPts, vector<Point2f>& nextPts ){
  double maxDist = 25;
  // double maxError = 20;
  double maxThreshold = 10;
  Mat status;
  Mat temp = findFundamentalMat(prevPtsNonReduced, nextPtsNonReduced, FM_RANSAC, 3, 0.99,status);
...
Mat takeVideo(VideoCapture cap, int pyrLevel, int skip, VideoWriter& writer){
  //variables
  Mat grayScale;
  Mat fund;
  ...
  // {
      ...
      if (emptyFrame){
        ...
        fund = reduce(previousPoints, nextPoints, reducedPrev, reducedNext);
        ...
        Mat homLeft, homRight;
        stereoRectifyUncalibrated(reducedPrev, reducedNext, fund, currentFrame.size(), homLeft, homRight);

        cout << "fundamental matrix: \n" << fund << endl;

        cout << "H1:  " << homLeft << "H2:  " << homRight; 
